...
<form class="variations_form cart" action="<?php echo esc_url(apply_filters('woocommerce_add_to_cart_form_action', $product->get_permalink())); ?>" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint($product->get_id()); ?>" data-product_variations="<?php echo $variations_attr; // WPCS: XSS ok. 
	...
	<?php if (empty($available_variations) && false !== $available_variations) : ?>
		<p class="stock out-of-stock"><?php echo esc_html(apply_filters('woocommerce_out_of_stock_message', __('This product is currently out of stock and unavailable.', 'utenzo'))); ?></p>
	<?php else : ?>
				...
				<?php foreach ($attributes as $attribute_name => $options) : ?>
					<tr>
						...
						<td class="value">
							 ...
							 */
							echo end($attribute_keys) === $attribute_name ? wp_kses_post(apply_filters('woocommerce_reset_variations_link', '<a class="reset_variations" href="#" aria-label="' . esc_attr__('Clear options', 'utenzo') . '">' . esc_html__('Clear', 'utenzo') . '</a>')) : '';
							?>
						</td>
					</tr>
				<?php endforeach; ?>
						...
						<div class="bt-attributes--value bt-value-color">
							<?php foreach ($options as $option) : ?>
								<div class="bt-js-item bt-item-color" data-value="<?php echo esc_attr($option); ?>">
									<div class="bt-color">
										<span style="background-color: <?php echo esc_attr($option); ?>;">
