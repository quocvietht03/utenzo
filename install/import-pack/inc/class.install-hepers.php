...
class Import_Pack_Plugin_Installer_Helper extends Import_Pack_Installer_Helper {
	...
	public static function install( $plugin, $force = false ) {
		...
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'utenzo' )
				)
			)
	...
	public static function activate( $plugin ) {
		...
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'utenzo' ), $plugin['name'] )
				)
			);
    }
	...
	public static function bulk_activate( $plugins ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'utenzo' ), WP_PLUGIN_DIR ),
			);
		}
		$status = array();
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'utenzo' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'utenzo' ), wp_remote_retrieve_body( $request ) );
				}
			}
